...
A downstream bridge, `bevel_share.py` / `hc-bevel`, runs a clipboard-based two-way exchange with Bevel Intelligence. It has no Bevel API credentials and performs no automatic upload. Setup mode sends execution-relevant chapters, asks Bevel to create/update its `HealthCoach Operating Context` File, and installs a Monday 04:25 Check-in for the complete Monday-through-Sunday week that just ended. Weekly mode requests a marker-delimited `HEALTHCOACH_BEVEL_WEEKLY_V1` JSON package with seven ordered days and field-level provenance. Verify mode reads that reply from the clipboard, validates dates/types/ranges/origins, rejects locked-plan conflicts, retrieves local evidence for Bevel's observations and proposed changes, generates a source-bound audit, writes the latest exchange into Part V of the one canonical report, and copies a verified response back to Bevel. The report generator preserves that marker-bounded ledger across regeneration; no sidecar exchange file is created, and only the newest 12 weeks are retained. Workout mode asks Bevel Strength Builder to parse exactly the three defined lifts. Wearable results still enter through Bevel-supported data sources; the user explicitly pastes plan and verification prompts at the trust boundary.
...
- an interactive 91-item supplement evidence audit plus a separate 47-item searchable experimental/drug/peptide/gray-market queue (`supplement_audit.py` / `hc-supplements`);
...
| `test_retrieval.py` | No-LLM wiring smoke test, including folder assertions for focused nootropic, experimental, and locked whole-food topics. |
...
| `hc-refresh-nootropics` | Focused acquisition wrapper for ALCAR, citicoline, uridine, Noopept, and bromantane; then rebuilds and smoke-tests LanceDB. |
| `hc-refresh-experimental` | Parallel, resumable A/B-source top-up across configured `08_peptides_gray` and `05_fat_loss_drugs` topics; then rebuilds and smoke-tests LanceDB. |
| `hc-refresh-whole-foods` | Focused acquisition wrapper for the eleven user-locked culinary foods; then rebuilds and smoke-tests LanceDB. |
| `report_navigator.py` | Standard-library curses navigator for chapter/page selection, full-text search, and keyboard reading of the canonical report. |
| `hc-report` | Executable launcher for the report navigator. |
...
- study from 04:50 for 60 minutes, split into Bible/Jesus study then Anki/technical study when faith study is selected;
...
`./hc-supplements` launches `supplement_audit.py`. The default intake is a nine-step guided curses assessment: goals/current activity; training timing/injuries; current supplements/results; medicines/safety; supplements and the preferred food/product intake route; experimental/peptide/gray items to investigate; daily food/sleep/caffeine/substances; food/home/faith/alternative ideas; and stores/buying/deadline. Every screen has a plain-language description of what the answer changes. Arrow keys or `j/k` move, Space toggles, `/` filters long lists, `r` restores that section's full list, `a` selects all visible entries, `n` clears, and Enter advances. Common defaults are preselected. The `locked_values` path makes three immutable profile facts visually and behaviorally non-removable in both the quick and detailed intake: `creatine_monohydrate`, medication `tirzepatide`, and its required current-prescription evidence review. `hard_define_profile()` then validates every recorded key against the current option catalog, de-duplicates in catalog order, removes contradictory none/unknown sentinels when substantive answers exist, verifies every exclusive selector, and restores those locked facts for noninteractive callers as well. The single report prints `HC_SELECTION_LOCK_V1`, its validation result, stable keys, human labels, and whether each row is locked or user/default-confirmed; downstream synthesis must treat that table as the authoritative intake record and cannot infer unselected choices. Cardio timing, strength timing, supplement-source strategy, timeline, and the experimental-research boundary are mutually exclusive within their groups. The supplement-source choices are `whole_food_first`, `mixed`, and `products_allowed`; whole-food first is the default. A deterministic route map labels ordinary food routes, foods that contain related compounds but are not form/dose-equivalent, the locked creatine-product exception, and candidates with no practical whole-food equivalent. The chosen route appears in the profile, stack-change table, focused source-strategy table, and complete evidence inventory. It never converts a food into proof of efficacy or treats a missing food route as permission to buy a product. If selected answers are ambiguous or safety-relevant, the program asks for at most one consolidated detail line; the default path has no text entry. A final summary offers generate, restart, or cancel before models load. The store category is nonexclusive, so a user may choose any combination of supported warehouse, grocery/pharmacy/specialty/online channels. The older typing-heavy intake is available only with `--detailed-assessment`. The 91 supplement candidates remain in their user-submitted discovery queues; the searchable 47-item experimental/incretin/peptide/gray catalog is research-only. The conservative default evaluates only manually selected gray items. An explicit `screen_strong_human` opt-in retrieves every configured experimental topic and surfaces entries only when at least two unique candidate-folder A/B sources with explicit human-participant and intervention/exposure signals survive. It separately counts safety-marked human sources, extracts chemistry/biology pathway flags, searches for direct co-use/interaction passages with the recorded stack, and prints `UNKNOWN / NOT VERIFIED` when compatibility is not directly established. Mechanistic plausibility never becomes a safe-together verdict, an active-stack addition, or a self-administration protocol. ALCAR and uridine are selectable supplement reviews, citicoline remains selectable, and Noopept and bromantane are selectable in the gray-market review screen. Coverage is recalculated from unique on-topic A/B human sources rather than trusting queue names. A separate scoped retrieval over concurrent-training, compressed-schedule, heat, sleep/circadian, injury, and recovery folders produces the tagged timing recommendation. The report also prints deterministic KEEP/REMOVE/REVIEW actions for the current stack and a capped ADD-candidate table; selected weak items stay visible but do not get promoted to strong evidence.

The same hard-definition path now fixes eleven explicit food-review keys: `beets`, `oregano`, `saffron_food`, `kimchi`, `tamarind`, `blueberries`, `garlic_food`, `ginger_food`, `honey_treats`, `cinnamon_food`, and `turmeric_food`. They cannot be removed by clearing a screen or by noninteractive `none`. `WHOLE_FOOD_CATALOG` retrieves them through a separate food-oriented query and report ledger, requires candidate-folder A/B passages with explicit human signals to raise coverage, and keeps culinary foods distinct from oils, extracts, capsules, and standardized trial products. These foods never enter the supplement shortlist. `hc-refresh-whole-foods` performs the one-time targeted source acquisition for their dedicated folders, rebuilds LanceDB, and runs the expanded folder checks.
...
`./hc-report` reads that same canonical file and creates no output. Its curses index supports arrow-key selection; Space/Enter opens a logical page; `/` searches chapter titles and paragraph content; Space pages down while reading; `[`/`]` changes chapters; and `b` returns to the index. Noninteractive helpers are `./hc-report --list`, `./hc-report --search "phrase"`, and `./hc-report --page 017`. Logical pages come from the `### CHAPTER … · PAGE …` headings inserted by `indexed_document()`.
...
After generation, browse the canonical report interactively:
